...
- Добавлена экспериментальная сегментированная запись напрямую в MP4 с отдельным M4A микрофона и возвратом к legacy-флоу через настройки.
- Добавлены профили общих лимитов 50/140/280/500 минут; профиль 140 минут / 3,6 GB выбран по умолчанию.
- Части автоматически закрываются по 55 минут или 1,8 GB, а общий лимит учитывает видео и микрофон.
- Добавлено бесшовное воспроизведение сегментов и обратный отсчет времени в основном окне и menu bar.
- Gemini получает все сегменты с валидацией дорожек, resumable-загрузкой и точечным переиспользованием неизменившихся файлов.
- Сайдбар показывает папки выше записей без папки; добавлено перемещение записей через контекстное меню и возврат в корень.
