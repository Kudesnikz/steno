...
class AIWorker(QThread):
    ...
    def __init__(self, session_base_path, config, agent_id):
        ...
        # We only need config for reading API key/url/model, not for writing tokens directly
        self.config = config 
    ...
    def run(self):
        ...
        try:
            ...
            # Проверка ключа
            if not api_key:
                self.finished_signal.emit(False, "AI Error", "Нет API ключа. Настройте ключ в меню.", "", self.agent_id, 0)
                return
            ...
            self.finished_signal.emit(True, "Готово!", f"Протокол сохранен: {os.path.basename(txt_path)}", base_name, self.agent_id, total_tokens)

        except Exception as e:
            logger.exception("AI Worker failed")
            base_n = os.path.basename(self.session_base_path) if hasattr(self, 'session_base_path') else ""
            self.finished_signal.emit(False, "AI Ошибка", str(e)[:100], base_n, self.agent_id, 0)
        
        finally:
            # Удаление с облака
            if 'ready_files' in locals():
                for uf in ready_files:
                    try: 
                        client.files.delete(name=uf.name)
                    except Exception as e:
                        logger.warning(f"Failed to delete file {uf.name}: {e}")
