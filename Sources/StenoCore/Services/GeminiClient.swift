...
public actor GeminiClient {
    ...
    public func generateReport(videoURL: URL, audioURLs: [URL], config: AppConfig, agent: Agent) async throws -> AIProcessingResult {
        ...
        do {
            for fileURL in files where FileManager.default.fileExists(atPath: fileURL.path) {
    ...
    private func upload(fileURL: URL, apiKey: String, baseURL: String) async throws -> GeminiFile {
        ...
        startRequest.httpBody = try encoder.encode(StartUploadRequest(file: StartUploadFile(displayName: fileURL.lastPathComponent)))
        ...
        let (responseData, response) = try await urlSession.data(for: uploadRequest)
    ...
    private func waitUntilReady(file: GeminiFile, apiKey: String, baseURL: String) async throws -> GeminiFile {
        var current = file
    ...
    private func generateContent(files: [GeminiFile], videoURL: URL, config: AppConfig, agent: Agent) async throws -> GenerateContentResponse {
        ...
        let meetingDate = meetingDate(from: videoURL)
        let prompt = """
        Внимательно изучи прикрепленные файлы и пришли ответ в формате markdown, строго следуя System instructions.

        Дата встречи: \(meetingDate). Если тебе необходимо указывать имена участников - проверь правильность их написания в разных частях видео. Очень важно чтоб имена были корректными. Ответ без дополнительных комментариев.
        """
    ...
    private func meetingDate(from videoURL: URL) -> String {
