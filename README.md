...
*   **`scripts/build_arch.sh [all|x86_64|arm64]`** — Основной скрипт сборки.
    ...
    *   По окончании сборки автоматически удаляет временные директории (`build/`, `dist/`), оставляя только готовый `.dmg`.
*   **`scripts/build_dmg.sh`** — Упаковщик и Подписыватель.
    *   Вызывает `scripts/fix_frameworks.py` для исправления структуры бандлов PyQt6, гарантируя правильную работу на других Mac.
    *   Подписывает все внутренние компоненты и сам бандл приложения (Apple Code Signing) с поддержкой Hardened Runtime.
...
**1. Сборка для обеих архитектур (Apple Silicon + Intel) (По умолчанию):**
Запустите скрипт без аргументов или передайте `all`. Скрипт последовательно соберет версию для `arm64`, упакует её, очистит кэш и соберет версию для `x86_64`.
```bash
./scripts/build_arch.sh
```
Результат: Два файла `Steno-arm64.dmg` и `Steno-x86_64.dmg` в корне проекта.

**2. Сборка только для Intel (x86_64):**
Явно укажите архитектуру. На Apple Silicon машине потребуется установленная Rosetta 2 (скрипт автоматически создаст отдельное окружение `.venv-x86` используя `arch -x86_64`).
```bash
./scripts/build_arch.sh x86_64
```
Результат: `Steno-x86_64.dmg` в корне проекта.

**3. Сборка только для Apple Silicon (M1/M2/M3):**
